...
export default function App() {
          ...
          storage.getPages().then((loadedPages) => {
            setPages((currentPages) => {
              ...
              const mergedPages = loadedPages.map(loadedPage => {
                ...
                if (pendingSaves.current.has(loadedPage.id)) return currentPage;
              ...
              const ghostPages = currentPages.filter(cp => 
                !loadedPages.some(lp => lp.id === cp.id) && pendingSaves.current.has(cp.id)
              );
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Load settings from Tauri Store (no API call)
      try {
        ...
        if (settings) {
          ...
          if (settings.spellCheck !== undefined) setIsSpellCheckEnabled(settings.spellCheck);
          if (settings.dailyWordTarget !== undefined) setDailyWordTarget(settings.dailyWordTarget);
          ...
          if (savedSpellCheck !== null) setIsSpellCheckEnabled(JSON.parse(savedSpellCheck));
        }
      } catch (error) {
  ...
  const filteredPages = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = pages.filter((p) => searchResults.includes(p.id));
    }
  ...
  const handlePageCreate = useCallback(async (template?: typeof BUILTIN_TEMPLATES[0]) => {
    if (activePageId) finalizePageTitle(activePageId);
    const now = Date.now();
    ...
    const newPage: Page = {
      ...
      isPinned: false, isManuallyEdited: false, templateName: template ? template.name : "Blank Page",
    };
    setSearchQuery("");
    ...
    try {
      ...
      setTimeout(() => pendingSaves.current.delete(id), 1000);
  ...
  const handleDuplicatePage = useCallback(async (id: string) => {
    ...
    const newPage: Page = {
      ...
      createdAt: now, updatedAt: now, isPinned: false, isManuallyEdited: true,
    };
    setPages(prev => [newPage, ...prev]);
    setActivePageId(newPage.id);
    await storage.savePage(newPage);
  ...
  const handlePagePin = useCallback(async (id: string) => {
    let updatedPage: Page | null = null;
    setPages((prev) => prev.map((p) => {
      if (p.id === id) { updatedPage = { ...p, isPinned: !p.isPinned }; return updatedPage; }
      return p;
    }));
    if (updatedPage) {
      try { await storage.savePage(updatedPage); } 
      catch (error) { setPages((prev) => prev.map(p => p.id === id ? { ...p, isPinned: !p.isPinned } : p)); }
    }
  }, []);

  const handlePageDelete = useCallback(async (id: string) => {
    setPages((prev) => {
      const updated = prev.filter((p) => p.id !== id);
      if (activePageId === id) setActivePageId(updated.length > 0 ? updated[0].id : null);
      return updated;
    });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === "n") { e.preventDefault(); handlePageCreate(); }
      if ((e.metaKey || e.ctrlKey) && e.key === "f") { e.preventDefault(); setIsFocusMode((prev) => !prev); }
      if ((e.metaKey || e.ctrlKey) && e.key === "p") { e.preventDefault(); if (activePageId) handlePagePin(activePageId); }
      if ((e.metaKey || e.ctrlKey) && e.key === "k") { e.preventDefault(); setIsShortcutPaletteOpen((prev) => !prev); }
      if ((e.metaKey || e.ctrlKey) && e.key === ",") { e.preventDefault(); setIsSettingsOpen(true); }
      if ((e.metaKey || e.ctrlKey) && e.shiftKey && e.key === "Backspace") { e.preventDefault(); if (activePageId) handlePageDelete(activePageId); }
      if ((e.metaKey || e.ctrlKey) && e.shiftKey && e.key === " ") { e.preventDefault(); setIsQuickCaptureOpen(true); }
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [handlePageCreate, handlePagePin, activePageId, handlePageDelete]);

  useEffect(() => {
    const handleBeforeUnload = () => { if (activePageId) finalizePageTitle(activePageId); };
    window.addEventListener("beforeunload", handleBeforeUnload);
    return () => window.removeEventListener("beforeunload", handleBeforeUnload);
  }, [activePageId, finalizePageTitle]);

  const debouncedSave = useMemo(() => debounce(async (page: Page) => {
    pendingSaves.current.add(page.id);
    setSyncStatus(prev => ({ ...prev, [page.id]: 'saving' }));
    try { await storage.savePage(page); setSyncStatus(prev => ({ ...prev, [page.id]: 'synced' })); } 
    catch (error) { setSyncStatus(prev => ({ ...prev, [page.id]: 'error' })); } 
    finally { setTimeout(() => pendingSaves.current.delete(page.id), 1000); }
  }, 1000), []);
  ...
  const handleTitleChange = useCallback((title: string, isManual: boolean = true) => {
    if (!activePageId) return;
    setPages((prev) => {
      const updated = prev.map((p) => {
        if (p.id === activePageId) {
          const updatedPage = { ...p, title, updatedAt: Date.now(), isManuallyEdited: isManual ? true : p.isManuallyEdited };
          debouncedSave(updatedPage);
          searchManager.updatePage(updatedPage);
          return updatedPage;
        }
  ...
  const handleQuickCaptureSave = async () => {
    if (!quickCaptureContent.trim()) { setIsQuickCaptureOpen(false); return; }
    const now = Date.now();
    const newPage: Page = {
      id: uuidv4(), title: "Quick Capture", content: quickCaptureContent,
      createdAt: now, updatedAt: now, isPinned: false, templateName: "Quick Capture",
    };
    setPages((prev) => [newPage, ...prev]);
    setActivePageId(newPage.id);
    setSearchQuery("");
  ...
  // DOCX export using local generation (no API)
  const handleExportDOCX = async () => {
    if (!activePage) return;
    try {
      // Simple fallback: save as .txt file instead of DOCX
      const blob = new Blob([activePage.content], { type: 'text/plain' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${activePage.title || "Untitled"}.txt`;
  ...
  return (
    <ErrorBoundary>
      <div className="flex h-screen w-screen overflow-hidden bg-[var(--bg-color)] text-[var(--text-color)] transition-colors duration-300">
        <div className={cn("h-full w-72 shrink-0 transition-all duration-300 ease-in-out", isFocusMode ? "-ml-72" : "ml-0")}>
          <Sidebar
            pages={filteredPages}
          ...
          <div className="flex-1 overflow-hidden">
            {activePage ? (
              ...
              <div className="flex h-full flex-col items-center justify-center p-8 text-center">
                ...
                <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Welcome to Flow</h2>
                <p className="mt-2 max-w-xs text-sm text-zinc-500 dark:text-zinc-400">A minimalist, distraction-free space for your thoughts.</p>
                <button onClick={() => setIsTemplatePickerOpen(true)} className="mt-8 rounded-full bg-zinc-900 px-6 py-2.5 text-sm font-medium text-white transition-colors hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200">
                  Create your first page
                </button>
              </div>
            )}
          </div>
          <StatusBar wordCount={wordCount} charCount={charCount} lineCount={lineCount} target={dailyWordTarget} />
        </main>

        <ShortcutPalette isOpen={isShortcutPaletteOpen} onClose={() => setIsShortcutPaletteOpen(false)} />
        <Settings
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/20 backdrop-blur-sm dark:bg-black/40">
            <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="grid grid-cols-2 gap-4 p-6 md:grid-cols-3">
                <button onClick={() => handlePageCreate()} className="flex flex-col items-start rounded-xl border border-zinc-100 p-4 text-left transition-all hover:border-zinc-300 dark:border-zinc-800 dark:hover:border-zinc-700">
                  <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-zinc-100 text-zinc-500 dark:bg-zinc-800"><Plus size={20} /></div>
                ...
                {BUILTIN_TEMPLATES.map((template) => (
                  <button key={template.name} onClick={() => handlePageCreate(template)} className="flex flex-col items-start rounded-xl border border-zinc-100 p-4 text-left transition-all hover:border-zinc-300 dark:border-zinc-800 dark:hover:border-zinc-700">
                    <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-orange-50 text-orange-500 dark:bg-orange-950/30"><FileText size={20} /></div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/20 backdrop-blur-sm dark:bg-black/40">
            <div className="w-full max-w-lg overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
                ...
                <kbd className="rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[10px] font-medium text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400">Ctrl + Enter to save</kbd>
              </div>
              <textarea
                autoFocus
                ...
                onKeyDown={(e) => {
                  if (e.ctrlKey && e.key === "Enter") handleQuickCaptureSave();
                  if (e.key === "Escape") setIsQuickCaptureOpen(false);
                }}
              ...
              <div className="flex justify-end border-t border-zinc-100 bg-zinc-50 px-4 py-2 dark:border-zinc-800 dark:bg-zinc-900/50">
                <button onClick={() => setIsQuickCaptureOpen(false)} className="mr-2 rounded-md px-3 py-1.5 text-xs font-medium text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-800">Cancel</button>
                <button onClick={handleQuickCaptureSave} className="rounded-md bg-zinc-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200">Save Note</button>
              </div>
            </div>
          </div>
        )}
